...
int main()
{
  ...
  while(t--) // Loop through each test case
  {
    ...
    vector<long long> a(n); // Array to store the efficiency of n-1 teams
    for (long long i = 1; i <= n - 1; i++)
    {
      cin >> a[i]; // Loop to read the efficiencies of n-1 teams
    }

    //INPUTS

    long long sum = 0; // Variable to  store the sum of the efficiencies
    for (long long i = 1; i <= n - 1; i++)
    {
      sum += a[i]; // Add each team's efficiency to the sum
    }

    long long n_efficiency = -(sum); 
    cout << n_efficiency << endl; // Output the efficiency of the missing team
  }
