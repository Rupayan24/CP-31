...
int main()
{
  ...
  while(t--)
  {
    ...
    for (int i = 0; i < n; i++)
    {
      if(a[i] == k)  // A subsegment(subarray) can also be of 1 element ---> so if the element is present in the array and then in that 1 size subsegment it becomes the most commom element --> thus the ans will be yes.
      {
        present = true;
        break;
      }
    ...
    {
      cout << "No" << endl; // If the element is not present in the array --> cout "no"
    }
  }
